...
mongosh /tmp/database/migrate-analysis-manifest-to-be-by-omic-unit.js
    ...
    Generates an annotation manifest for each omic unit in all analyses
    with matching dataset source and version.  
    
    docker exec -it rosalution-rosalution-db-1 mongosh --file /tmp/fixtures/migrations/migrate-analysis-manifest-to-be-by-omic-unit.js
...
try {
  ...
  analyses.forEach((element) => {
    if ('manifest' in element) {
      const omicUnits = [];
      for (const unit of element['genomic_units']) {
        if ('gene' in unit) {
          omicUnits.push({'type': 'gene', 'unit': unit['gene']});
        }

        if ('variants' in unit) {
          for (const variant of unit['variants']) {
            omicUnits.push({'type': 'hgvs_variant', 'unit': variant['hgvs_variant']});
          }
        }
      }

      const originalManifest = element['manifest'];
      const newManifest = [];
      for (const unit of omicUnits) {
        const unitsManifest = {
          'unit': unit['unit'],
          'manifest': [],
        };

        for (const originalEntry of originalManifest) {
          const datasetName = Object.keys(originalEntry);
          const name = datasetName[0];
          if (datasetsByType[unit['type']].includes(...datasetName)) {
        ...
        newManifest.push(unitsManifest);
      }

      newManifest.map(((originalEntry) => {
