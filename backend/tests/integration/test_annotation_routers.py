...
def test_get_annotations_by_gene(client, mock_access_token, database_collections):
    ...
    database_collections['genomic_unit'].collection.find_one.return_value = read_test_fixture("annotations-VMA21.json")
    response = client.get("annotate/gene/VMA21", headers={"Authorization": "Bearer " + mock_access_token})

    print("THIS IS THE RESPONSE AND ITS CONTENT")
    print(response);
...
#     database_collections['genomic_unit'].collection.find_one.return_value = read_test_fixture("annotations-HGVS-Variant.json")
#     response = client.get("annotate/hgvsVariant/NM_001017980.3:c.164G>T", headers={"Authorization": "Bearer " + mock_access_token})
