...
def test_http_annotation_base_url(http_annotation_transcript_id):
    ...
    assert (
        actual
        == "http://grch37.rest.ensembl.org/vep/human/hgvs/NM_170707.3:c.745C>T?content-type=application/json;"
    )
...
def test_http_annotation_task_build_url(http_annotation_task_many_datasets):
    ...
    expected = (
        "http://grch37.rest.ensembl.org/vep/human/hgvs/"
        "NM_170707.3:c.745C>T?content-type=application/json;refseq=1;"
    )
    assert actual == expected


def test_annotation_task_create_http_task(
...
def fixture_http_annotation_transcript_id(
    http_annotation_task_empty, transcript_id_dataset
):
...
def fixture_transcript_related_datasets(annotation_collection):
    ...
    return list(
        filter(
            lambda x: (
                x["data_set"] == "transcript_id"
                or x["data_set"] == "SIFT Prediction"
                or x["data_set"] == "SIFT Score"
            ),
...
def fixture_http_annotation_many_datasets(
    http_annotation_task_empty, transcript_datasets_json
):
