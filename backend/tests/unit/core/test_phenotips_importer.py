...
def test_import_genomic_unit_data_hgvs(phenotips_importer):
    ...
    data = {"transcript": "NM_001005484.1", "cdna": "c.1036C>T"}
    actual = phenotips_importer.import_genomic_unit_collection_data(
        data, "hgvs")
    assert actual['hgvs_notation'] == "NM_001005484.1:c.1036C>T"
...
def test_import_genomic_unit_data_gene(phenotips_importer):
    ...
    actual = phenotips_importer.import_genomic_unit_collection_data(
        data, "gene")
    assert actual == {"id": "", "gene_symbol": "BRCA1", "annotations": {}}


def test_import_analyses_data(phenotips_importer, exported_phenotips_to_import_json):
    ...
    actual = phenotips_importer.import_analyses_data(
        exported_phenotips_to_import_json, variant_data)
