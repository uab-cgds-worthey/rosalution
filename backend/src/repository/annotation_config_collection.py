...
class AnnotationConfigCollection:
    ...
    async def find_by_data_set(self, dataset_name):
        """Returns a data set source that matches by name"""
        return await self.collection.findOne({"data_set": dataset_name})

    async def datasets_to_annotate_by_type(self, types):
        """gets dataset configurations according to the types"""
        configuration = await self.all()
        return [dataset for dataset in configuration if dataset["genomic_unit_type"] in types]

    async def datasets_to_annotate_for_units(self, genomic_units_to_annotate):
        """
        ...
        datasets_to_annotate = await self.datasets_to_annotate_by_type(types_to_annotate)
